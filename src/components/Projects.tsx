...
const projects: Project[] = [
  {
    ...
    tags: ["Azure", "Foundry", "OpenAI", "PDF", "ContainerApp", "PowerAutomate"],
  },
  {
    industry: "Finance",
...
export const Projects = () => {
  ...
  return (
    <TooltipProvider>
      <section className="h-full w-full relative overflow-y-auto flex flex-col justify-center p-4 sm:p-6 lg:p-8 bg-white">
        {/* Subtle animated geometric shapes */}
        <div className="absolute top-1/3 right-16 w-32 h-32 border-2 border-primary/20 rounded-full animate-slow-pulse" style={{ animationDuration: '15s' }}></div>
        <div className="absolute bottom-1/3 left-20 w-24 h-24 border-2 border-primary/20 rounded-lg rotate-12 animate-slow-spin" style={{ animationDuration: '19s' }}></div>
        
        <div className="container mx-auto px-4 sm:px-8 lg:px-16 relative">
        <div className="text-center mb-6 sm:mb-8 w-full max-w-7xl mx-auto">
          <div ref={titleAnimation.ref} className={titleAnimation.className}>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-2 sm:mb-3 font-ultra-thick">
          ...
          <div ref={subtitleAnimation.ref} className={subtitleAnimation.className}>
            <p className="text-xs sm:text-sm lg:text-base text-muted-foreground max-w-2xl mx-auto leading-tight">
              Real business problems, solved with automation and AI.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5 max-w-7xl mx-auto">
          {projects.map((project, index) => {
            ...
            return (
              <div key={index} ref={projectAnimation.ref} className={projectAnimation.className}>
                <div className="group h-full flex flex-col bg-gradient-to-br from-background to-muted/30 rounded-xl p-4 sm:p-5 border-2 border-muted hover:border-primary/30 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                  <div className="mb-2">
                  ...
                  <h3 className="text-base sm:text-lg font-bold text-foreground mb-1.5 group-hover:text-gradient transition-all duration-300">
                    {project.title}
                  </h3>
